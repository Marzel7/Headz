...
describe("[Challenge] Free Rider", function () {
  ...
  before(async function () {
    ...
    await ethers.provider.send("hardhat_setBalance", [
      ...
      //"0x8670e9ec6598c0000", // 15 ETH for testing
    ]);
  ...
  it("Exploit", async function () {
    ...
    this.freeRiderAttack = await (
      await ethers.getContractFactory("FreeRiderAttack", buyer)
    ).deploy(
      attacker.address, // partner
      this.nft.address,
    ...
    // console.log(fromWei(await ethers.provider.getBalance(this.freeRiderAttack.address)));
    await this.freeRiderAttack.connect(attacker).execute([0, 1, 2, 3, 4, 5]);
  });
