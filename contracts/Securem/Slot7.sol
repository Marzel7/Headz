...
contract InSecureumDAO is Pausable, ReentrancyGuard {
    ...
    function join() external payable onlyWhenOpen nonReentrant {
        require(
            ...
            "InSecureumDAO: Incorrect ETH amount"
        );
        members[msg.sender] = true;
    }

    function createVote(uint256 _voteId, uint8[] memory _possibleOutcomes)
        external
        onlyWhenOpen
        whenNotPaused
    {
    ...
    function castVote(uint256 _voteId, uint8 _vote)
        external
        voteExists(_voteId)
        onlyWhenOpen
        whenNotPaused
    ...
    /* only deletes membership for msg.sender */
