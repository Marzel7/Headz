...
contract WalletDeployer {
    ...
    function can(address u, address a) public view returns (bool) {
        assembly { 
            let m := sload(0)                            // load from storage slot 0
            if iszero(extcodesize(m)) {return(0, 0)}     // if m has no code, return data at 0 offset, of 0 bytes
            let p := mload(0x40)                        // load from memory, offset 64 bytes
