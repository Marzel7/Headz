...
constructor(address payable gnosisSafeAddress, address gnosisSafeProxyFactoryAddress, address walletRegistryAddress, address tokenAddress) {
    gnosisSafe = GnosisSafe(gnosisSafeAddress);
    gnosisSafeProxyFactory = GnosisSafeProxyFactory(gnosisSafeProxyFactoryAddress);
...
function execute(address[] calldata users, address attacker) external {
    ...
    GnosisSafeProxy proxy = gnosisSafeProxyFactory.createProxyWithCallback(address(gnosisSafe), initializer, 0, walletRegistry);
